...
from .services import ExerciseService, MuscleService
...
class MusclesView(APIView):
    ...
    @extend_schema(
        request=MuscleCreateSerializer, responses={201: MuscleGroupSerializer}
    )
    def post(self, request):
        ...
        return Response(
            MuscleGroupSerializer(muscle).data, status=status.HTTP_201_CREATED
        )
    ...
    def patch(self, request):
        ...
        return Response(
            MuscleGroupSerializer(muscle).data, status=status.HTTP_202_ACCEPTED
        )
...
class ExerciseView(APIView):
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(name="name", type=str, description="Filter by name"),
            OpenApiParameter(
                name="muscle", type=str, description="Filter by muscle slug"
            ),
            OpenApiParameter(
                name="primary", type=bool, description="Additional filter for muscles"
            ),
            OpenApiParameter(
                name="difficulty", type=int, description="Filter by difficulty level"
            ),
            OpenApiParameter(
                name="equipment", type=str, description="Filter by equipment slug"
            ),
        ],
    ...
    @extend_schema(
        request=ExerciseCreateSerializer, responses={201: ExerciseSerializer}
    )
    def post(self, request):
    ...
    def patch(self, request):
        ...
        exercise = ExerciseService.update_exercise(
            int(m_id), **serializer.validated_data
        )
        ...
        return Response(
            ExerciseSerializer(exercise).data, status=status.HTTP_200_ACCEPTED
        )
