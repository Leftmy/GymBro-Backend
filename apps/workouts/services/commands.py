...
class WorkoutCommands:
    ...
    def create_workout(
        *,
        ...
        with transaction.atomic():
            ...
            plan_exercises = WorkoutCommands._build_plan_exercises(workout, exercises)
    ...
    @staticmethod
    def update_workout(workout_id: int, **data: Any) -> WorkoutPlan:
        """
    ...
    def assign_workout_to_user(
        *,
        ...
        with transaction.atomic():
            ...
            if is_active:
                UserWorkoutPlan.objects.filter(user=user, is_active=True).update(
                    is_active=False
                )
    ...
    def update_user_workout_plan(
        user_workout_plan_id: int, **data: Any
    ) -> UserWorkoutPlan:
        """
    ...
    def _build_plan_exercises(
        ...
        exercise_map = {e.slug: e for e in Exercise.objects.filter(slug__in=slugs)}
