...
from apps.exercises.models import Exercise, ExerciseMuscleGroup, MuscleGroup
from apps.users.models import UserRole
from apps.workouts.models import UserWorkoutPlan, WorkoutPlan, WorkoutPlanExercise
    ...
    def handle(self, *args, **kwargs):
        ...
        for name, svg in muscles_data:
            mg, _ = MuscleGroup.objects.get_or_create(
                name=name, defaults={"svg_id": svg, "slug": slugify(name)}
            )
            muscle_objs[name] = mg
            ...
            if created:
                for m_name in ex_data["muscles"]:
                    ExerciseMuscleGroup.objects.create(
                        exercise=exercise, muscle_group=muscle_objs[m_name]
                    )
        ...
        for p_name in plan_names:
            plan, created = WorkoutPlan.objects.get_or_create(
                ...
                defaults={
                    "created_by": trainer,
                    "description": f"A great {p_name} routine \
                        created by our lead trainer.",
                },
            )
        ...
        if first_plan:
            UserWorkoutPlan.objects.get_or_create(
                user=client, workout_plan=first_plan, defaults={"is_active": True}
            )
