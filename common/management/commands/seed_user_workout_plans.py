...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        fixture_path = (
            Path(__file__).resolve().parents[2] / "fixtures" / "user_workout_plans.json"
        )
        ...
        if options["flush"]:
            ...
            self.stdout.write(
                self.style.WARNING("Existing user workout plans deleted.")
            )
        ...
        for item in items:
            ...
            if item.get("started_at"):
                started_at = datetime.strptime(item["started_at"], "%Y-%m-%d").date()
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Assigned {workout_plan.name} to {user.username}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"Plan already assigned: {user.username} -> {workout_plan.name}"
                    )
                )

        self.stdout.write(
            self.style.SUCCESS(f"\nDone. Created {created_count} user workout plans.")
        )
